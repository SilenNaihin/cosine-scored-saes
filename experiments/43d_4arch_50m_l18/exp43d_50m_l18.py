"""
Experiment 43d: 50M token 4-architecture comparison at L18 (saprmarks recipe)
==============================================================================

L18 is middle depth with moderate activation norms (~98). This is where
exp40/42c ran at 500M, so 50M results here provide a direct convergence
comparison. Also tests whether NoC dead features emerge at 50M (exp42c
showed emergence at ~25M at 500M scale).
...
Run on <gpu-server> GPU 1:
    ...
    CUDA_VISIBLE_DEVICES=1 PYTHONUNBUFFERED=1 nohup python3 experiments/exp43d_50m_l18.py \
        2>&1 | tee experiments/exp43d_output.log &
"""
...
SAVE_DIR = Path("/mnt/nvme0/checkpoints/exp43d")
RESULTS_PATH = Path("experiments/exp43d_results.json")
