"""
Experiment 30: Learning Rate Sweep at 50M Tokens
==================================================

Fairness control at production scale. All prior comparisons use LR=3e-4.
If the cosine encoder has a different optimal LR than the standard encoder,
the comparison is unfair.

Run at 50M tokens (not 5M) because exp17 showed qualitative differences
between 5M and 50M at L27: cosine went from worse than standard to +8pp
better. LR sensitivity could change with scale, so the sweep must match
the production comparison.

Sweep LR in {1e-4, 3e-4, 1e-3} for standard and adaptive_l2 at L27.
...
Usage:
    ...
    CUDA_VISIBLE_DEVICES=0 PYTHONUNBUFFERED=1 python3 experiments/exp30_lr_sweep_50m.py 2>&1 | tee experiments/exp30_output.log
"""
...
# --- Output ---
SAVE_DIR = "checkpoints/exp30_lr"
RESULTS_PATH = "experiments/exp30_lr_results.json"
...
def main():
    print("Experiment 30: Learning Rate Sweep (50M tokens)")
    print("=" * 70)
    ...
    if std_best and adp_best:
        ...
        if std_best[0] == adp_best[0]:
            print(f"    FAIR — both prefer LR={std_best[0]:.0e} at 50M tokens")
            print(f"    All prior comparisons at LR=3e-4 are valid.")
        else:
            print(f"    CAUTION — standard prefers {std_best[0]:.0e}, adaptive prefers {adp_best[0]:.0e}")
            print(f"    Prior comparisons at shared LR=3e-4 may be unfair.")
            ...
            print(f"    Gap at per-variant best LR: {gap:+.4f} (adaptive - standard)")
            # Check if the gap still favors adaptive
            if gap > 0:
                print(f"    Adaptive STILL wins even at per-variant best LR.")
            else:
                print(f"    Standard wins at per-variant best LR — cosine advantage may be LR artifact!")
