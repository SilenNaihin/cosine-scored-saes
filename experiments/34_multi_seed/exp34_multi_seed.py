"""
Experiment 34: Multi-Seed Variance Estimation (50M tokens)
============================================================

Every result in this project is a single run. A reviewer will ask:
"Are the gaps real or within run-to-run variance?"

3 SAE variants × 3 seeds × L27 = 9 runs at 50M tokens.

Variants:
  standard       — BatchTopKSAE (inner-product baseline)
  adaptive_l2    — AdaptiveCosineBatchTopKSAE with norm-adaptive init (our best)
  perfeature_l2  — PerFeatureAdaptiveCosineSAE with norm-adaptive init

Seeds: 42, 123, 456

If the cosine advantage is >2 std from zero → statistically robust.
If within noise → need to scale up or revise claims.

Note: Cosine variants use norm-adaptive init (scale_b = log(mean(||x||)))
from exp27. This is critical for L27 where norms=407 vs sqrt(d)=64.

Estimated runtime: ~9 hours on H100 (9 × ~1 hour per variant).

Usage:
    ssh <server>     cd ~/MechInter--RNH
    CUDA_VISIBLE_DEVICES=1 PYTHONUNBUFFERED=1 python3 experiments/exp34_multi_seed.py 2>&1 | tee experiments/exp34_output.log
"""
...
# --- Output ---
SAVE_DIR = "checkpoints/exp34_multi_seed"
RESULTS_PATH = "experiments/exp34_results.json"
...
def main():
    print("Experiment 34: Multi-Seed Variance Estimation (50M tokens)")
    print("=" * 70)
    ...
    print(f"  Norm-adaptive scale_b init: log({mean_norm:.1f}) = {math.log(mean_norm):.4f}")
    print(f"  vs sqrt(d) init: log({math.sqrt(D_MODEL):.1f}) = {math.log(math.sqrt(D_MODEL)):.4f}")
