"""
Experiment 25c: Mistral-7B RMSNorm Positive Control
====================================================

Part of the 2x2 normalization matrix (exp25a/b/c):
  - exp25a: LayerNorm model A (negative control)
  - exp25b: LayerNorm model B (negative control)
  - exp25c: Mistral-7B RMSNorm (positive control) <-- THIS

This is the RMSNorm model -- we expect STRONG cosine advantage matching
Qwen3-8B. On Qwen3-8B (RMSNorm), cosine SAEs consistently beat standard:
+8 FVE at L27, 3.3x more alive features, cos>inner 70-90%.
...
Run on <gpu-server> (NOT <gpu-server>). No CUDA_VISIBLE_DEVICES needed
(single GPU box).

Usage:
    ssh <server>     cd ~/MechInter--RNH
    nohup .venv/bin/python -u experiments/exp25c_mistral7b_rmsnorm.py > experiments/exp25c_output.log 2>&1 &
"""
...
def main():
    print("Experiment 25c: Mistral-7B RMSNorm Positive Control")
    print("=" * 70)
    ...
    # =========================================================================
    print(f"\n{'='*70}")
    print(f"  SUMMARY TABLE -- Mistral-7B (RMSNorm), 5M tokens")
    print(f"{'='*70}")
    ...
    # Key comparison: RMSNorm positive control
    print(f"\n  KEY COMPARISON -- RMSNorm positive control:")
    print(f"  On Qwen3-8B (RMSNorm): cosine SAEs get +8 FVE, 3.3x alive, cos>inner 70-90%")
    print(f"  On Mistral-7B (RMSNorm) -- expect STRONG cosine advantage matching Qwen3-8B:")
    for layer_idx in LAYERS:
        lr = all_results["layers"].get(str(layer_idx), {})
