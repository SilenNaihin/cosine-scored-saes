"""
Experiment 25b: Falcon-7B LayerNorm Control
============================================
...
IMPORTANT: Run on GPU 1 only.
    CUDA_VISIBLE_DEVICES=1 python -u experiments/exp25b_falcon7b_layernorm.py

Usage:
    ssh <server>     cd ~/MechInter--RNH
    CUDA_VISIBLE_DEVICES=1 nohup .venv/bin/python -u experiments/exp25b_falcon7b_layernorm.py > experiments/exp25b_output.log 2>&1 &
"""
...
# --- Output ---
SAVE_DIR = "checkpoints/exp25b"
RESULTS_PATH = "experiments/exp25b_results.json"
...
# =============================================================================
# Activation Collection (Falcon-specific hooks)
# =============================================================================
...
def _collect_layer_acts(model, layer_idx, inputs):
    """Capture residual stream activations at a Falcon layer via forward hook."""
    captured = {}
...
def ablate_feature_kl(model, activation, feature_dir, layer_idx):
    ...
    h = model.transformer.h[layer_idx].register_forward_hook(make_hook(x))
    with torch.no_grad():
        try:
    ...
    h = model.transformer.h[layer_idx].register_forward_hook(make_hook(x_ablated))
    with torch.no_grad():
        try:
...
def main():
    print("Experiment 25b: Falcon-7B LayerNorm Control")
    print("=" * 70)
    ...
    # =========================================================================
    print(f"\n{'='*70}")
    print(f"  SUMMARY TABLE — Falcon-7B (LayerNorm), 5M tokens")
    print(f"{'='*70}")
    ...
    print(f"  On Falcon-7B (LayerNorm):")
    for layer_idx in LAYERS:
        lr = all_results["layers"].get(str(layer_idx), {})
