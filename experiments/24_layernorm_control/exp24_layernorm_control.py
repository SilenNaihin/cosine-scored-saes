"""
Experiment 24: LayerNorm Negative Control (Pythia-2.8B)
======================================================
...
IMPORTANT: Run on GPU 1 only (GPU 0 is running exp22).
    CUDA_VISIBLE_DEVICES=1 python -u experiments/exp24_layernorm_control.py

Usage:
    ssh <server>     cd ~/MechInter--RNH
    CUDA_VISIBLE_DEVICES=1 nohup .venv/bin/python -u experiments/exp24_layernorm_control.py > experiments/exp24_output.log 2>&1 &
"""
...
DEVICE = "cuda"
MODEL_DTYPE = torch.float32   # Pythia uses float32
...
# --- Output ---
SAVE_DIR = "checkpoints/exp24"
RESULTS_PATH = "experiments/exp24_results.json"
...
def main():
    print("Experiment 24: LayerNorm Negative Control (Pythia-2.8B)")
    print("=" * 70)
    ...
    # =========================================================================
    print(f"\n{'='*70}")
    print(f"  SUMMARY TABLE — Pythia-2.8B (LayerNorm), 5M tokens")
    print(f"{'='*70}")
    ...
    print(f"  On Pythia-2.8B (LayerNorm):")
    for layer_idx in LAYERS:
        lr = all_results["layers"].get(str(layer_idx), {})
